...
def get_ndvi_with_diagnostics(geometry, days_back_options=(14, 30, 60), cloud_thresholds=(20, 40, 80)):
    """
    Same as get_ndvi, but also returns real diagnostic metadata:
      - cloud_pct: actual CLOUDY_PIXEL_PERCENTAGE of the image(s) used
        (not the threshold - the real measured value)
      - last_image_date: acquisition date of the most recent image
        actually used in the mean
      - window_days_used: which fallback window succeeded
    This is used to build an honest "data quality" panel rather than
    just presenting a number with no provenance.
    """
    end = datetime.datetime.utcnow()
    ...
    for days_back, cloud_thresh in zip(days_back_options, cloud_thresholds):
        ...
        # Pull real diagnostics from the most recent image in this collection
        ...
        last_image_date = (
            datetime.datetime.utcfromtimestamp(millis / 1000).date().isoformat()
            if millis else None
        )
        ...
        return {
            ...
            "window_days_used": days_back,
        }

    return {"value": None, "cloud_pct": None, "last_image_date": None, "window_days_used": None}
...
def get_ndvi_thumbnail_url(geometry, days_back_options=(14, 30, 60), cloud_thresholds=(20, 40, 80)):
    """
    Returns a PNG thumbnail URL of the NDVI image over the field area,
    for display on an interactive map. Returns None if no image found.
    """
    end = datetime.datetime.utcnow()
    ...
    region = geometry.buffer(1500).bounds()
    ...
    for days_back, cloud_thresh in zip(days_back_options, cloud_thresholds):
        ...
        vis_params = {
            "min": 0.0, "max": 0.8,
            "palette": ["#a50026", "#f46d43", "#fee08b", "#66bd63", "#1a9850"],
        }
        try:
            url = ndvi_image.getThumbURL({
                "region": region, "dimensions": 512, **vis_params
            })
            return url
        except Exception:
            return None
...
def get_field_snapshot(lat: float, lon: float, project_id: str):
    ...
    as a single dict, ready to feed into the scoring engine. Also
    includes real data-quality diagnostics (cloud %, last satellite
    pass date) sourced from the actual NDVI fetch, not estimated.
    """
    init_earth_engine(project_id)
    geometry = get_field_geometry(lat, lon)

    ndvi_diag = get_ndvi_with_diagnostics(geometry)

    return {
        "lat": lat,
        "lon": lon,
        "soil_moisture": get_soil_moisture(geometry),
        "ndvi": ndvi_diag["value"],
        "land_surface_temp_c": get_land_surface_temp(geometry),
        "timestamp": datetime.datetime.utcnow().isoformat(),
        "data_quality": {
            "cloud_pct": ndvi_diag["cloud_pct"],
            "last_satellite_pass_date": ndvi_diag["last_image_date"],
            "ndvi_window_days_used": ndvi_diag["window_days_used"],
        },
    }
